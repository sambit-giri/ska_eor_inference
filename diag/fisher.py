...
ell_lim = 8 # number of ell bins to choose starting from the smallest bin
...
# limiting the Fisher analysis to the single (diagonal or off-diagonal) terms only
diag_data = np.diagonal(fid_data, offset=0, axis1=-2, axis2=-1)
diag_derivs = np.diagonal(MAPS_derivs, offset=0, axis1=-2, axis2=-1)

# Flattening the diagonal data along last two axes
shp = diag_data.shape
flat_data = np.reshape(diag_data, shape=(shp[0], int(shp[1]*shp[2])), order='C')
flat_derivs = np.reshape(diag_derivs, shape=(nparams, shp[0], int(shp[1]*shp[2])), order='C')

print("Shape of the flattened concatenated arrays")
print(flat_data.shape)
print(flat_derivs.shape)

# Fisher analysis steps
errcov = np.cov(flat_data, rowvar=False)

im = plt.imshow(errcov, origin='lower', cmap='coolwarm', vmax=1., vmin=-1.)
plt.colorbar(im)
plt.gcf().set_size_inches(6,6)
plt.savefig('covar_diag_1000_AA4.png', format='png', dpi=300, bbox_inches='tight')
plt.close()
plt.clf()

print(f"Condition number for the corresponding covariance matrix is: {np.log10(np.linalg.cond(errcov)):.4f}")

inv_cov = np.linalg.inv(errcov)

mean_derivs = np.mean(flat_derivs, axis=1)

x = np.matmul(mean_derivs, inv_cov)
#print(x.shape)
F = np.matmul(x, mean_derivs.T)
...
samples = np.arange(300, n_samp+5, 5)
...
for k, sample_size in enumerate(samples):
    derv_ = np.mean(flat_derivs[:,:sample_size,:], axis=1)
    data_ = flat_data[:sample_size,:]
