...
statname = 'PdPS' # Name of the statistic (e.g., 'ps' for power spectrum)
nbins = 10  # Number of k-bins for the spherical power spectrum
    ...
    if compute:
        ...
        for i in tqdm.tqdm(range(n_samp)):
            ...
            if ('FID' in fname):
                ...
                # noisy data
                ps_obs[i], ks = estimate_PdPS(
                                dt_obs, nbins, 
                                box_length, box_length_los
                            )
                # noise
                ps_noise[i], ks = estimate_PdPS(
                                noise_lc, nbins, 
                                box_length, box_length_los
                            )
