...
ell_lim = 8 # number of ell bins, starting from the smallest
...
freq1 = int(nfreq//2) # channel number 1
freq2 = int(freq1 -1) # channel number 2

derv_ = np.mean(MAPS_derivs[:,:,:,freq1,freq2], axis=1)
data_ = fid_data[:,:,freq1,freq2]
...
plt.savefig('covar_diag_100_AAstar.png', format='png', dpi=300, bbox_inches='tight')
plt.close()
plt.clf()
...
for k, sample_size in enumerate(samples):
    derv_ = np.mean(MAPS_derivs[:,:sample_size,:,freq1,freq2], axis=1)
    data_ = fid_data[:sample_size,:,freq1,freq2]
    errcov = np.cov(data_, rowvar=False)
    inv_cov = np.linalg.inv(errcov)
    x = np.matmul(derv_, inv_cov)
    #print(x.shape)
...
plt.title('Convergence of the Parameters Variances')
